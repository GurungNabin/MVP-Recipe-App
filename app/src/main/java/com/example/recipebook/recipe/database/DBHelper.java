...
package com.example.recipebook.recipe.database;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public List<Recipe> getAllRecipe(int limit, int offset) {
        List<Recipe> recipes = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = null;
    ...
    public void deleteRecipe(Recipe recipe) {
        Log.d("DB_DEBUG", "Deleting recipe with ID: " + recipe.getId());

        SQLiteDatabase db = this.getWritableDatabase();
        int rowsDeleted = db.delete(TABLE_NAME, COLUMN_ID + " = ?", new String[]{String.valueOf(recipe.getId())});
        db.close();

        if (rowsDeleted > 0) {
            Log.d("DB_SUCCESS", "Recipe deleted successfully: " + recipe.getName());
        } else {
            Log.e("DB_ERROR", "Failed to delete recipe: " + recipe.getName());
        }
    }
